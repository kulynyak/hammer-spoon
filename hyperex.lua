...
CHyper.new = function(triggerKey)
  ...
  if _self._triggerKey ~= nil then
    if CHyperParasites.realFlagMask[_self._triggerKey] ~= nil then
      ...
      local hotkeyDown = function()
        _self:enter()
      end
      local hotkeyUp = function()
        _self:exit()
      end
      _self._trigger = hs.hotkey.bind(
        _self._triggerMod,
        _self._triggerKey,
        0,
        hotkeyDown,
...
CHyperParasites._handleTap = function(e)
  ...
  if hyper ~= nil then
    ...
    if (realFlags and mask) == mask then
...
CHyperParasites.startTap = function()
  if CHyperParasites._tap == nil then
    CHyperParasites._tap = hs.eventtap.new(
...
local CHyperStickyImpl = {
  ...
  chain = function(self)
    if self.chainTimer == nil then
      self.chainTimer = hs.timer.delayed.new(self.chainDelay, function()
        self:exitSticky()
      end)
    end
...
CHyperImpl.sticky = function(self, mode, op)
  ...
  if type(self.stickyMode) == 'number' then
    if self._stickyModal == nil then
      ...
      self._stickyModal:bind({}, 0x35, 0, function()
        self:exitSticky()
      end, nil, nil)
    end
