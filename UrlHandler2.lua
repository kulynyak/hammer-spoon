...
local function mapByFn(keyFn, valFn, base, override)
  ...
  local tmp = {}
  for _, item in ipairs(mix) do
    tmp[keyFn(item)] = valFn(item)
  end
  local result = {}
...
local function installedBrowsers(base, overrides)
  local keyFn = function(x)
    return x.id
  end
  local valFn = function(x)
    return x
  end
...
local function mkLabel2BundleMap(browsers)
  ...
  for _, item in ipairs(browsers) do
    if item.label == nil then
      obj.logger.ef(
        "Key 'label' have to be defined for item: %s",
        hs.inspect(item)
      )
    else
...
function obj:_showChooser(labels, url)
  showChooser(
    self._label2BundleMap,
    labels,
    url,
    handleSelectBrowserAction,
    self.widgetWidth
  )
end
